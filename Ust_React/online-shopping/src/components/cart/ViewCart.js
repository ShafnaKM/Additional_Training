...
const ViewCart = () => {
  let [cartDetails, updateCartDetails] = useState({
    cartItems: [
      {
        cartId: 1,
        productId: 1,
      ...
      <div className="row">
        <div className="col-12">
          <div className="jumbotron">
            ...
            <p className="lead">
              You can checkout any of the following products.
            </p>
            <hr className="my-4" />
          </div>
        </div>
        ...
        {isCartEmpty() && (
          <>
            {cartDetails.cartItems.map((cartItem) => (
              <ViewCartItemCard
                cartItem={cartItem}
      ...
      {isCartEmpty() && (
        <div>
          <Modal show={showModal} scrollable={true}>
            ...
            <Modal.Body>
              {cartDetails.cartItems.map((cartItem) => (
                <ViewOrdderItemCard cartItem={cartItem} key={cartItem.cartId} />
              ))}
              <h2>Cart Total : {cartDetails.totalCartValue}</h2>
              ...
              <ToastContainer position="middle-center">
                <Toast show={showToast} onClose={() => setShowToast(false)}>
                  ...
                  <Toast.Body>
                    Your order has been placed successfully. Redirecting to view
                    orders page.
                  </Toast.Body>
                </Toast>
              </ToastContainer>
